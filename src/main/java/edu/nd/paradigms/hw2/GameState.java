...
import static edu.nd.paradigms.hw2.GameStatus.*;
...
public class GameState {
     ...
     * 6 guesses for the user to start with, and setting the game status to {@link GameStatus#IN_PROGRESS}.
     */
    public GameState() {
     ...
     */
    protected GameState(Dictionary guessDictionary, String answer, int guessesRemaining, GameStatus gameStatus) {
     ...
     * <ul>
     * <li>{@link GameStatus#IN_PROGRESS} - the game is ongoing</li>
     ...
     * @throws GameAlreadyOverException if the game is already over (see {@link GameState#isGameOver()}
    ...
    /**
     * All methods below validate constructor inputs to ensure their validity (that is, it is possible
     * to have a Wordle game in the state specified).
     *
     * Hint to students: Do not modify or use this functions (either this one or the functions below)
     */
    ...
    /**
     * Ensures that the guess dictionary is not empty (that is, the player has at least one word they can guess)
     * @param guessDictionary the dictionary of allowed guesses
     */
    ...
    /**
     * Ensures the answer actually exists in the guess dictionary
     * @param answer The target word the player is trying to guess
     * @param guessDictionary The dictionary of words the player is allowed to guess
     */
    ...
    /**
     * Ensures that the number of guesses remaining at the start of the game is valid (that is, non-negative)
     * @param guessesRemaining the starting number of guesses remaining
     */
    ...
    /**
     * Validate that the game status is consistent with the number of guesses remaining. For example, it's not
     * possible to lose a game but have a positive number of guesses remaining.
     * @param gameStatus the current status of the Wordle game.
     * @param guessesRemaining the number of guesses remaining to the user
     */
    private static void validateGameStatus(GameStatus gameStatus, int guessesRemaining) {
        ...
        if (guessesRemaining == 0 && gameStatus == IN_PROGRESS) {
